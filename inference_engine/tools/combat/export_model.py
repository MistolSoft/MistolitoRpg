...
OUTPUT_DIR = os.path.join(MODEL_DIR, "combat")

BACKBONE_ONNX = os.path.join(OUTPUT_DIR, "backbone.onnx")
BACKBONE_ESPDL = os.path.join(OUTPUT_DIR, "backbone.espdl")
...
def load_model():
    if not os.path.exists(CHECKPOINT_PATH):
        ...
        print("Ejecuta combat_train_model.py primero")
        sys.exit(1)
...
def export_actor_init(model, num_actions=3):
    import struct
    path = os.path.join(OUTPUT_DIR, "actor_init.bin")
...
def export_critic(model):
    import struct
    path = os.path.join(OUTPUT_DIR, "value_head.bin")
...
def main():
    ...
    print(f"  {os.path.join(OUTPUT_DIR, 'actor_init.bin')}")
    print(f"  {os.path.join(OUTPUT_DIR, 'value_head.bin')}")
