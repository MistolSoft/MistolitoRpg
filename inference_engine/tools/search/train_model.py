...
class SearchModelSplit(nn.Module):
    def __init__(self, num_actions=4):
        super().__init__()
...
def main():
    ...
    dataset_path = os.path.join(script_dir, "search_dataset.csv")
    if not os.path.exists(dataset_path):
        dataset_path = os.path.join(project_dir, "search_dataset.csv")
    if not os.path.exists(dataset_path):
        dataset_path = "search_dataset.csv"
    ...
    model = SearchModelSplit().to(DEVICE)
    ...
    checkpoint_path = os.path.join(model_dir, "search_model.pth")
